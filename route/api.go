...
func SetupAPI(address string) error {
	...
	sellerRoutes := r.Group("/seller", middleware.AuthoriseSeller())
	{
		...
		sellerRoutes.GET("/products", seller.SellerProducts)
		sellerRoutes.PUT("/products/:id", seller.UpdateSellerProduct)
		sellerRoutes.DELETE("/products/:id", seller.DeleteSellerProduct)
		sellerRoutes.GET("/purchases", seller.PurchasesSeller)
	}
